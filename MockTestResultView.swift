...
struct MockTestResultView: View {
    ...
    private var scoreCard: some View {
        VStack(spacing: 16) {
            ...
            HStack {
                ...
                Text("\(MockTest.passingScore)/\(questionsPerTest)")
    ...
    private var testDetails: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 12) {
                DetailRow(icon: "calendar", label: "Date", value: test.formattedDate)

                if test.timerEnabled {
                    DetailRow(icon: "timer", label: "Time Taken", value: test.formattedTime)
                }

                DetailRow(icon: "checkmark.circle", label: "Correct Answers", value: "\(test.score)")
                DetailRow(icon: "xmark.circle", label: "Incorrect Answers", value: "\(test.incorrectAnswers.count)")
            }
            .padding()
    ...
    private var categoryBreakdown: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            let sortedCategories = test.categoryBreakdown.keys.sorted { $0.displayName < $1.displayName }
...
struct CategoryStatRow: View {
    ...
    var accuracyColor: Color {
        if stats.accuracy >= 80 { return .green }
        if stats.accuracy >= 60 { return .orange }
        return .red
    }
...
struct ReviewIncorrectView: View {
    ...
    private var incorrectQuestions: [(question: Question, userAnswer: UserAnswer)] {
        test.incorrectAnswers.compactMap { answer in
            guard let question = test.questions.first(where: { $0.id == answer.questionID }) else { return nil }
            return (question, answer)
        }
    }

    private var current: (question: Question, userAnswer: UserAnswer)? {
        guard !incorrectQuestions.isEmpty, currentIndex < incorrectQuestions.count else { return nil }
            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            Label(current.question.category.displayName, systemImage: current.question.category.icon)
                                .font(.caption)
                                .foregroundColor(.secondary)
...
struct ReviewAnswerButton: View {
    ...
    var backgroundColor: Color {
        ...
        if isUserAnswer { return .red.opacity(0.2) }
