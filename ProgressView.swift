...
struct ProgressView: View {
    ...
    private var isPremium: Bool {
        dataManager.hasUnlockedFullAccess
    }
    ...
    private var allCategoryStats: [(category: QuestionCategory, correct: Int, total: Int, accuracy: Double)] {
        ...
        return categoryTotals.map { category, stats in
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        Text("Avg:")
                            .font(.caption2)
                            .foregroundColor(.secondary)
                        Text(String(format: "%.1f/50", averageScore))
                            .font(.caption.bold())
                    }
